...
// PrintLog 提示日志类输入日志,并且流向下一个对象方法
...
// Write 错误日志类写日志方法
func (ErrorL *ErrorLogger) Write(message string) {
	fmt.Printf("Error Logger out:%s.\n", message)
}

// PrintLog 错误日志类输入日志,并且流向下一个对象方法
func (ErrorL *ErrorLogger) PrintLog(level int, message string) {
	if ErrorL.Level == level {
		ErrorL.Write(message)
	}
	if ErrorL.NextLogger != nil {
		ErrorL.NextLogger.PrintLog(level, message)
	}
}

// SetNextLogger 错误日志类设置下一个对象方法
func (ErrorL *ErrorLogger) SetNextLogger(logger BaseLogger) {
	ErrorL.NextLogger = logger
}
